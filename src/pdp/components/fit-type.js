...
export const FitType = (props) => {
  const large = useMediaQuery("(min-width:1024px) and (max-width:1279px)");
  const fitTypeCount = props.product.fit < 1;
  const formButton = {
    ...
    padding: large ? "0 0.75rem" : "0 0.875rem"
  };
  const formButtonSelected = {
    ...
    padding: large ? "0 0.6875rem" : "0 0.8125rem"
  };
  ...
  function getOptions() {
    if (runOnce) {
      setListOption(props.product.fit);
      setRunOnce(false);
      setRestOfOptions(props.product.fit.slice(1, props.product.fit.length));
    }
    return;
  }
  getOptions();
